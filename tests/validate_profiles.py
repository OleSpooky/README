...
def validate_profile_execution(profile_name):
    ...
    try:
        ...
        # I shape: allow either (M, N) or (N,) depending on implementation
        if hasattr(I, "shape"):
            if I.ndim == 1:
                if I.shape[0] != params['N']:
                    return False, f"I shape {I.shape} doesn't match N={params['N']}"
            elif I.ndim >= 2:
                if I.shape[1] != params['N']:
                    return False, f"I shape {I.shape} doesn't match N={params['N']}"
        else:
            return False, "I has no shape attribute (unexpected type)"
        ...
        fraction_negative = float(np.mean(neg_mask))
...
def run_validation():
    ...
    if 'high_temperature_observer' in [p.get('name') for p in profiles]:
